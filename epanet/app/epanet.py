...
def get_controls(client: ModbusTcpClient) -> dict:
    ...
    try:
        return {'pipe_statuses': client.read_coils(0, 100).bits, 'pump_settings': client.read_holding_registers(0, 100).registers}
    except Exception as e:
...
def read_data(en: epanet) -> dict:
    ...
    try:
        return {'junction_pressures': [float(en.getNodePressure(i)) for i in en.getNodeJunctionIndex()], 'tank_heads': [float(en.getNodeHydraulicHead(i)) for i in en.getNodeTankIndex()], 'pump_flows': [float(en.getLinkFlows(i)) for i in en.getLinkPumpIndex()]}
    except Exception as e:
...
def main():
    ...
    try:
        ...
        print(f"Failed to run the program due to an unexpected error: {e}")
        sys.exit(1)
    finally:
        if 'client' in locals():
            client.close()
